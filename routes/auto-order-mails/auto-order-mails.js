...
const decryptPayload = (text) => {
  try {
    ...
    logInfo(
      "Auto Order Mail",
      `กำลังถอดรหัส Token: ${text.substring(0, 20)}...`,
    );
...
const getDataForStation = async (lic_code, autoItem) => {
  try {
    logInfo(
      "Auto Order Mail",
      `อ่านข้อมูลปั๊ม: ${autoItem.ptrl_desc} (${autoItem.ptrl_number})`,
    );
    ...
    const [orderResult, manager] = await Promise.all([
      pgConn.getWithParams(
        dbPrefix + lic_code,
        orderScript,
        [autoItem.ptrl_number],
        config.connectionString(),
      ),
    ...
    logInfo(
      "Auto Order Mail",
      `   ผู้จัดการปั๊ม: ${manager ? manager.emp_code : "ไม่พบ"}`,
    );
    return {
      lic_code: lic_code,
      ...
      manager_email: cleanEmails(autoItem.ptrl_remark)
        ? "prattananien@gmail.com, puautarm@gmail.com"
        : "",
    ...
    logError(
      "Auto Order Mail",
      `Auto Order Mail Data Error ${autoItem.ptrl_number}`,
      err,
    );
    return null;
  }
...
const sendAutoOrderEmail = async (stationData) => {
  try {
    ...
    const reportBaseUrlProd =
      process.env.IS_PROD == "true"
        ? "https://aos-qa.bangchak.co.th/main/order/order-report"
        : "https://spd-demo.dtc.co.th:9101/main/order/order-report";
    ...
    if (hasOrders) {
      ...
      logInfo(
        "Auto Order Mail",
        "ไม่มียอดสั่งซื้อ: เตรียมส่งเมลแจ้งเตือนไม่มียอดสั่งน้ำมัน",
      );
    }

    const htmlContent = generateFullEmailHtml(
      stationName,
      confirmUrl,
      rowsHtml,
      hasOrders,
    ...
    const result = await mailer.sendMail(
      [
        "prattananien@gmail.com",
        "ornwara.traiyavudh02@gmail.com",
        "puautarm@gmail.com",
      ],
      subject,
      htmlContent,
...
const processAutoOrderMails = async (lic_code) => {
  ...
  logInfo(
    "Auto Order Mail",
    `เริ่มประมวลผล: ${moment().format("YYYY-MM-DD HH:mm:ss")}`,
  );
  try {
    const query = `
    ...
    return {
      success: true,
      message: `พบข้อมูล ${mailDataList.length} แห่ง`,
      data: mailDataList,
    };
  } catch (err) {
...
exports.getAutoOrderMailData = async (req, res) => {
  try {
    ...
    if (!lic_code)
      return sendResponse(res, "error", "-1", "พารามิเตอร์ไม่ถูกต้อง", []);

    const result = await processAutoOrderMails(lic_code);
    if (!result.success)
      return sendResponse(res, "error", "-3", result.message, []);
...
exports.runAutoOrderMailTask = async (lic_code = "") => {
  ...
  try {
    ...
    const stationDataResults = await Promise.all(
      autoList.map((item) => getDataForStation(lic_code, item)),
    );
...
const processEmailBatches = async (lic_code, stationList) => {
  ...
  for (let i = 0; i < stationList.length; i += BATCH_SIZE) {
    ...
    logInfo(
      "Auto Order Mail",
      `[Batch] กลุ่ม ${i / BATCH_SIZE + 1}: สถานีลำดับที่ ${i + 1} - ${Math.min(i + BATCH_SIZE, stationList.length)} จาก ${stationList.length}`,
    );

    const results = await Promise.all(
      batch.map((station) => processStationEmail(lic_code, station)),
    );
  ...
  try {
    ...
    if (!decrypted) {
      return sendResponse(
        ...
        "error",
        "-2",
        "ไม่สามารถถอดรหัส token ได้ (อาจจะ Key ผิด หรือ Token ไม่ครบ)",
        [],
      );
    }
...
const runAutoOrderToSapTask = async (lic_code = "") => {
  ...
  logInfo(
    "Auto Order SAP Background",
    `เริ่มตรวจสอบรายการ Auto Order ประจำรอบเวลา ${moment().format("HH:mm:ss")}`,
  );

  try {
    console.log(
      `Auto Order SAP Background [${lic_code}] เริ่มตรวจสอบรายการ Auto Order ประจำรอบเวลา ${moment().format("HH:mm:ss")}`,
    );
    ...
    if (result.code || !result.data || result.data.length === 0) {
      logInfo(
        "Auto Order SAP Background",
        "ไม่พบรายการออเดอร์อัตโนมัติที่ค้างส่งในรอบนี้",
      );
    ...
    console.log(
      `Auto Order SAP Background [aosQA] พบออเดอร์ระบบอัตโนมัติจำนวน ${result.data.length} รายการ`,
    );

    for (const order of result.data) {
      // logInfo('Auto Order SAP Background', `กำลังส่งออเดอร์ ID: ${order.order_id} ของสถานี: ${order.ptrl_desc}`);
      console.log(
        `Auto Order SAP Background [aosQA] กำลังส่งออเดอร์ ID: ${order.order_id} ของสถานี: ${order.ptrl_desc}`,
      );
      ...
      if (sapResult && sapResult[0] && sapResult[0].status === "success") {
        ...
        console.log(
          `Auto Order SAP Background [aosQA] ส่งออเดอร์ ID: ${order.order_id} ของสถานี: ${order.ptrl_desc} สำเร็จ`,
        );
      } else {
        // logError('Auto Order SAP Background', `   ❌ ส่งออเดอร์ ID: ${order.order_id} ล้มเหลว: ${sapResult[0]?.message || 'Unknown Error'}`);
        console.log(
          `Auto Order SAP Background [aosQA] ส่งออเดอร์ ID: ${order.order_id} ของสถานี: ${order.ptrl_desc} ล้มเหลว`,
        );
      }
    }
    ...
    logError(
      "Auto Order SAP Background",
      "เกิดข้อผิดพลาดในการทำงานของ Task Auto Order To SAP",
      error,
    );
    return { success: false };
  }
  ...
  try {
    ...
    for (const petrol of petrolResult.data) {
      const stockScript = `
            ...
            `;
      const stockResult = await pgConn.getWithParams(
        dbPrefix + lic_code,
        stockScript,
        [petrol.ptrl_code],
        config.connectionString(),
    ...
    logInfo(
      "Auto Stock Logs",
      `รันเสร็จสิ้น: พบสถานีที่ไม่มีข้อมูล Stock จำนวน ${missingStockCount} แห่ง`,
    );
...
exports.runAutoOrderCleanupTask = async (lic_code = "") => {
  ...
  try {
    ...
    if (result.rowaction > 0) {
      logInfo(
        "Auto Order Cleanup",
        `อัปเดตออเดอร์ที่หมดอายุจำนวน ${result.rowaction} รายการ (ก่อนวันที่ ${thresholdDate})`,
      );
    } else {
      logInfo(
        "Auto Order Cleanup",
        "ไม่พบรายการที่ต้องลบสำหรับออเดอร์ที่มีอายุเกิน 3 วัน",
      );
    }
  } catch (error) {
...
`;
    ...
    logInfo(
      "Auto Order Mail",
      `updateAutoOrderFlag success: rowaction=${result.rowaction}`,
    );
