...
exports.getRunoutReportInformation = async (req, res, next) => {
  ...
  try {
    ...
    if (ptrl_code && ptrl_code.toString().toUpperCase() !== "ALL") {
      ...
      conditions.push(
        `(p.ptrl_code = $${params.length} OR p.ptrl_number = $${params.length})`,
      );
    }

    if (start_date) {
      params.push(
        moment(start_date).startOf("day").format("YYYY-MM-DD HH:mm:ss"),
      );
    ...
    if (act_val === "GROUP") {
      ...
      conditions.push(`ri.ptrl_code IN ${managerCodeIn}`);
    }
    ...
    console.log(script);
    const tbl_temporary = await pgConn.getWithParams(
      dbName,
      script,
      params,
      config.connectionString(),
    ...
    const countScript = `
        ...
        `;
    const tbl_temporary0 = await pgConn.getWithParams(
      dbName,
      countScript,
      params,
      config.connectionString(),
    ...
    xglobal.sendResponse(
      res,
      "error",
      "-1",
      "ไม่สามารถดึงข้อมูลได้: " + err.message,
    );
  }
};
