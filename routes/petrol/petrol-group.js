...
exports.getPetrolGroupInformation = async (req, res, next) => {
  ...
  return (async () => {
    ...
    if (
      ptrl_group_sales_org &&
      Array.isArray(ptrl_group_sales_org) &&
      ptrl_group_sales_org.length > 0
    ) {
      const salesOrg = ptrl_group_sales_org
    ...
    if (
      ptrl_group_order_type &&
      Array.isArray(ptrl_group_order_type) &&
      ptrl_group_order_type.length > 0
    ) {
      const orderTypes = ptrl_group_order_type
        .map((val) => `'${String(val).replace(/'/g, "''")}'`)
        .join(", ");
      conditions.push(`(
